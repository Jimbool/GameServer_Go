...
func combineStructAndMethodName(structName, methodName string) string {
	return structName + mDelimeterOfObjAndMethod + methodName
}

func constructFullModuleName(moduleName string) string {
	return moduleName + mModuleSuffix
}

func constructFullMethodName(methodName string) string {
...
func RegisterFunction(structObject interface{}) {
	// 获取 mathutilObject 对应的 Type 和 Value
	...
	// 获取structObject中的以C_开始的方法
	for i := 0; i < reflectType.NumMethod(); i++ {
		// 获得方法名称
		methodName := reflectType.Method(i).Name

		// 判断是否为导出的方法
		if strings.HasPrefix(methodName, mMethodPrefix) {
			// 获得方法及其输入参数的类型列表
			...
			// 判断输出参数数量是否正确
			if len(outTypes) != 1 {
				panic(errors.New(fmt.Sprintf("%s对象的方法%s返回参数数量不正确", structName, methodName)))
			}

			// 判断返回值是否为ResponseObject
			outType := outTypes[0]
			if _, ok := outType.FieldByName("Code"); !ok {
				panic(errors.New(fmt.Sprintf("%s对象的方法%s返回参数类型不正确", structName, methodName)))
			}
			...
			// 添加到列表中
			mMethodMap[combineStructAndMethodName(structName, methodName)] = NewMethodAndInOutTypes(method, inTypes, outTypes)
		}
	}
}
...
func callFunction(clientObj *Client, requestObj *RequestObject) {
	...
	if methodAndInOutTypes, ok = mMethodMap[key]; !ok {
		...
		ResponseResult(clientObj, requestObj, GetResultStatusResponseObj(responseObj, NoTargetMethod))
		return
	}
	...
	if paramLength != inTypesLength {
		...
		ResponseResult(clientObj, requestObj, GetResultStatusResponseObj(responseObj, ParamNotMatch))
		return
	}
